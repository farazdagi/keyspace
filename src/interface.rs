use {
    super::{KeyspaceInterval, Node, NodeRef, ReplicationStrategy},
    std::hash::Hash,
};
...
pub trait Keyspace<N: Node, const REPLICATION_FACTOR: usize>: Sized {
    ...
    /// Migration plan has all the information needed to migrate data from
    /// one to another when a node is added or removed.
    type MigrationPlan;
    ...
    fn iter(&self, node: &N) -> impl Iterator<Item = KeyspaceInterval<'_, Self::NodeRef<'_>>>;
